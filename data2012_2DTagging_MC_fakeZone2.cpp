...
//vectors for 2D tagging study
	...
	vector<vector<TH1F*> > vRmpf_ZonePt_tmp = buildZonePtVectorH1(my2DTaggingPtBinning,"Rmpf_tmp",nbinsx,xlow,xup) ;
...
//*****************************************************************************************************
	...
	//protection contre les jets non matches
						...
						if(binZone!=-1) {
							...
							if(binZone==1) {
								vRmpf_ZonePt_tmp[binZone][bin2DTaggingPt]->Fill(Rmpf,miscevent_weight);
							}
...
//*****************************************************************************************************
//
//                                      creating a fake 2DTagging zone 2
//
//*****************************************************************************************************	

	for(int j=0; j<n2DTaggingPtBins; j++) {
		float nentries = vRmpf_ZonePt_tmp[1][j]->Integral();
		vRmpf_ZoneFlavourPt[1][0][j]->Scale(0.1*nentries/(vRmpf_ZoneFlavourPt[1][0][j]->Integral()));
		vRmpf_ZoneFlavourPt[1][1][j]->Scale(0.7*nentries/(vRmpf_ZoneFlavourPt[1][1][j]->Integral()));
		vRmpf_ZoneFlavourPt[1][2][j]->Scale(0.1*nentries/(vRmpf_ZoneFlavourPt[1][2][j]->Integral()));
		vRmpf_ZoneFlavourPt[1][3][j]->Scale(0.1*nentries/(vRmpf_ZoneFlavourPt[1][3][j]->Integral()));
		vRmpf_ZoneFlavourPt[1][4][j]->Scale(0.*nentries/(vRmpf_ZoneFlavourPt[1][4][j]->Integral()));
		vRmpf_ZoneFlavourPt[1][5][j]->Scale(1.*nentries/(vRmpf_ZoneFlavourPt[1][5][j]->Integral()));
	}

	string name;
	for(int j=0; j<n2DTaggingPtBins; j++) {
		vRmpf_ZonePt[1][j]=vRmpf_ZoneFlavourPt[1][0][j];
		vRmpf_ZonePt[1][j]->Add(vRmpf_ZoneFlavourPt[1][1][j]);
		vRmpf_ZonePt[1][j]->Add(vRmpf_ZoneFlavourPt[1][2][j]);
		vRmpf_ZonePt[1][j]->Add(vRmpf_ZoneFlavourPt[1][3][j]);
		vRmpf_ZonePt[1][j]->Add(vRmpf_ZoneFlavourPt[1][4][j]);
		name = "Rmpf_" + getZoneBinName(1) + "_" + my2DTaggingPtBinning.getName(j);
		vRmpf_ZonePt[1][j]->SetNameTitle(name.c_str(),name.c_str());
		name = "";
	}
...
//v2DTaggingPlan_FlavourPt[binFlavour][bin2DTaggingPt]
	...
	for(int j=0; j<n2DTaggingPtBins; j++) {
		for(int i=0; i<nflavours-1; i++) {//divide flavour distri by total distri
			...
			histoName = "images2DTagging/2DTaggingZones/beforeDividing/" + getFlavourBinName(i) + "_" + my2DTaggingPtBinning.getName(j) + "_fakeZone2.pdf";
			...
			//name2DTaggingPlan_check += "_fakeZone2.pdf";
			...
			name2DTaggingPlan += "_fakeZone2.pdf";
...
//**********************************************************************************************************
	...
	for(int k=0; k<n2DTaggingPtBins; k++) {
		for(int i=0; i<nzones-2; i++) {
			...
			if(i==1) {
				v4x4MatrixPt[k](i,0) = 0.1;
				v4x4MatrixPt[k](i,1) = 0.7;
				v4x4MatrixPt[k](i,2) = 0.1;
				v4x4MatrixPt[k](i,3) = 0.1;
			}
		}
	}
	...
	TFile *out_matrix = new TFile("output_rootfile/outputMatrix2DTagging_MC_G_fakeZone2.root", "recreate");
	out_matrix->cd();
...
//*****************************************************************************************************
	...
	TFile *out = new TFile("output_rootfile/output2DTagging_MC_G_fakeZone2.root", "recreate");
